...
{
    class SkiillProfiWebAPI<T>
        where T : class, new()
    {

        public SkiillProfiWebAPI(string url, HttpClient client)
        {
            this.url = url;
            httpClient = client;
        }
        ...
        /// </summary>
        /// <param name="ex"></param>
        private void SetExeptionMessage(Exception ex)
        {
            if(ex != null)
                switch(ex)
                { 
                    ...
                    case HttpRequestException:
                        ...
                        if (((HttpRequestException)ex)?.StatusCode >= System.Net.HttpStatusCode.InternalServerError)
                    ...
                    case JsonSerializationException:
                        OnErrorMessage($"Ошибка преобазования данных.\n{ex.InnerException?.Message ?? ex.Message}");
                        break;
                    default:
                        throw ex;
                }
